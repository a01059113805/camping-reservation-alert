...
포항서숲오토캠핑장(`pscamp.hana-pnc.co.kr`) 예약 관리 페이지를 1시간마다 자동으로 확인해서, 새로 "예약완료(입금완료)"된 건이 있으면 휴대폰으로 무료 웹푸시 알림을 보내고, 동시에 구글 캘린더(전용 캘린더)에 일정을 등록한다. 반대로 이미 알렸던 예약이 취소되어 목록에서 사라지면 취소 알림을 보내고 등록해뒀던 캘린더 일정을 자동으로 지운다(이 기능 배포 이후 새로 확정되는 예약부터 적용됨 — 배포 전부터 있던 예약은 체크인 날짜/캘린더 일정 정보를 몰라 취소 감지 대상에서 빠진다). 맥북 배터리 절약을 위해 사람이 키보드/마우스를 10분 이상 안 건드리면(자리비움) 이 확인 자체를 건너뛴다.

## 알림에 담기는 정보
- 웹푸시 본문(신규 확정): `성함님 · 사이트 구역 및 번호 · 예약일 · 총 4명 (성인 2, 아동 2, 유아 0) · 010-0000-0000`
- 웹푸시 본문(취소): `성함님 · 사이트 구역 및 번호 · 체크인일 · 010-0000-0000` (삭제 직전 캘린더 일정 설명에서 읽어옴)
- 캘린더 일정 제목: `성함 · 사이트 구역 및 번호`
- 캘린더 일정 설명: 성함 / 연락처 / 인원 / 사이트 구역 및 번호 / 요금 / 특이사항(빈칸)

인원은 유아가 요금을 받지 않는 구분이라 성인/아동/유아를 0명이어도 항상 다 적는다.
요금은 알림창에서 굳이 볼 필요가 없어 캘린더 일정 설명에만 넣는다. 전화번호는 목록 페이지에서
바로 얻지만, 인원은 목록에 없어서 신규 확정 건마다 예약 상세 페이지를 한 번 더 열어 가져온다.
이 조회가 실패하면 해당 줄만 "확인 실패 (예약페이지 확인 필요)"로 표시되고, 알림과 캘린더
등록은 그대로 진행된다.

기존에 등록된 캘린더 일정을 새 설명 형식으로 일괄 갱신하려면
`python3 scripts/backfill_calendar_descriptions.py --dry-run` 으로 먼저 확인한 뒤
`--apply` 로 실행한다 (직접 적어 넣은 특이사항은 보존된다).
...
- `data/notified_ids.json` — 이미 알림을 보낸 예약번호별로 캘린더 일정 id/체크인일을 기록 (중복 알림/중복 캘린더 등록 방지 + 취소 감지·캘린더 일정 삭제에 사용, 로컬 launchd 실행이 자동으로 갱신/커밋)
...
3. 문제없으면 이후로는 그대로 1시간마다(자리비움 시 건너뜀) 자동 실행됨
